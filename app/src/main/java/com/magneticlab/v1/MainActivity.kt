...
@Composable private fun StatusCard(s: DeviceSnapshot) {
    Card(Modifier.fillMaxWidth(), colors = CardDefaults.cardColors(containerColor = Panel), shape = RoundedCornerShape(15.dp)) { Column(Modifier.padding(15.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) { Text("SYSTEM STATE", color = Cyan, fontSize = 11.sp, fontWeight = FontWeight.Bold); Text("LIVE", color = Green, fontSize = 10.sp, fontWeight = FontWeight.Bold) }
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) { Metric("BATTERY", "${s.battery}%", Green); Metric("RAM", "${s.ramUsed}%", Purple); Metric("STORAGE", "${s.storageUsed}%", Amber); Metric("TEMP", s.temperature, Cyan) }
    } }
}
...
@Composable private fun CommandRoom() {
    val context = LocalContext.current; val scope = rememberCoroutineScope(); var command by remember { mutableStateOf(TextFieldValue()) }; var output by remember { mutableStateOf("Ready. Commands execute through Android intents or documented system APIs.") }
    ScrollColumn {
        Eyebrow("LOCAL EXECUTION"); Text("Command", color = White, fontSize = 23.sp, fontWeight = FontWeight.Bold); Text("OPEN → RUN ACTION → VERIFY RESULT", color = Muted, fontSize = 12.sp)
        OutlinedTextField(value = command, onValueChange = { command = it }, modifier = Modifier.fillMaxWidth().height(145.dp), placeholder = { Text("Type a supported command…", color = Muted) }, leadingIcon = { Text(">", color = Cyan) })
        Text("SUPPORTED COMMANDS", color = White, fontSize = 12.sp, fontWeight = FontWeight.Bold)
        val examples = listOf("Open Wi-Fi settings", "Open Bluetooth settings", "Show battery level", "Vibrate for 1 second", "Open display settings")
        examples.forEach { example -> Button(onClick = { command = TextFieldValue(example) }, colors = ButtonDefaults.buttonColors(containerColor = Panel2), modifier = Modifier.fillMaxWidth()) { Text(example, color = White, modifier = Modifier.fillMaxWidth()) } }
        Button(onClick = { scope.launch { output = executeCommand(context, command.text) } }, modifier = Modifier.fillMaxWidth(), colors = ButtonDefaults.buttonColors(containerColor = Purple)) { Icon(Icons.Default.PlayArrow, "Run"); Spacer(Modifier.width(8.dp)); Text("RUN ON DEVICE") }
        OutputCard(output)
    }
}

private suspend fun executeCommand(context: Context, command: String): String = withContext(Dispatchers.Main) {
    val c = command.lowercase(Locale.US)
    try {
        when {
            c.contains("wi-fi") || c.contains("wifi") -> { context.startActivity(Intent(Settings.ACTION_WIFI_SETTINGS)); "✓ Wi-Fi settings opened by Android" }
            c.contains("bluetooth") -> { context.startActivity(Intent(Settings.ACTION_BLUETOOTH_SETTINGS)); "✓ Bluetooth settings opened by Android" }
            c.contains("display") || c.contains("brightness") -> { context.startActivity(Intent(Settings.ACTION_DISPLAY_SETTINGS)); "✓ Display settings opened by Android" }
            c.contains("battery") -> { "✓ ${readSnapshot(context).battery}% battery reported by BatteryManager" }
            c.contains("vibrate") -> { val vibrator = context.getSystemService(Vibrator::class.java); vibrator?.vibrate(VibrationEffect.createOneShot(1000, VibrationEffect.DEFAULT_AMPLITUDE)); "✓ Vibration requested through Vibrator API" }
            else -> "Unsupported command. No action was simulated."
        }
    } catch (e: SecurityException) { "Permission Required: ${e.message ?: "Android denied this action"}" } catch (e: Exception) { "Action failed: ${e.message ?: "unknown Android error"}" }
}
